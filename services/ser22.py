...
def change_password(correo,contrasena,nueva_contrasena):
    con = sqlite3.connect('db/vise0.db')
    cursor= con.cursor()
    query0=f"""UPDATE usuarios SET contrasena = '{nueva_contrasena}' WHERE correo = '{correo}' AND contrasena = '{contrasena}'"""
...
if status == 'OK':
    ...
    while True:
        ...
        ans = change_password(data["correo"], data["contrasena"],data["nueva_contrasena"])
